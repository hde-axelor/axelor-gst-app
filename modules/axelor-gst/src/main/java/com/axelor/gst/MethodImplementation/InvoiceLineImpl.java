...
public class InvoiceLineImpl implements InvoiceLineService {
	...
	public InvoiceLine calculateInvoiceLine(InvoiceLine invoiceLine) {
		...
		if (invoiceLine.getInvoice().getCompany().getAddress().getState().equals(invoiceLine.getInvoice().getInvoiceAddress().getState()) ||  invoiceLine.getInvoice().getCompany().getAddress().getState().equals(invoiceLine.getInvoice().getShippingAddress().getState())
			 )
			 {
			...
			invoiceLine.setSgst((invoiceLine.getNetAmount().multiply(invoiceLine.getGstRate())).divide(new BigDecimal(100)));
			invoiceLine.setCgst((invoiceLine.getNetAmount().multiply(invoiceLine.getGstRate())).divide(new BigDecimal(100)));
		...
		invoiceLine.setGrossAmount(invoiceLine.getNetAmount().add(invoiceLine.getIgst().add(invoiceLine.getCgst())));
